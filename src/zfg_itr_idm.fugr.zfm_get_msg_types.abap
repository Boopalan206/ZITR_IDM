FUNCTION zfm_get_msg_types.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IV_INPUT) TYPE  STRING
*"  EXPORTING
*"     VALUE(ET_RESULT) TYPE  ZTT_IDOC_MSG_TYPES
*"----------------------------------------------------------------------
  CONSTANTS:lv_function_id TYPE if_fdt_types=>id VALUE 'F10C74B575191EDFBD97275CE41CEBA7'.
  DATA:lv_timestamp      TYPE timestamp,
       lt_name_value     TYPE abap_parmbind_tab,
       ls_name_value     TYPE abap_parmbind,
       lr_data           TYPE REF TO data,
       lx_fdt            TYPE REF TO cx_fdt,
       la_zidoc_appl     TYPE if_fdt_types=>element_text,
...
****************************************************************************************************
  ls_name_value-name = 'ZIDOC_APPL'.
  la_ZIDOC_APPL = iv_input.
  GET REFERENCE OF la_ZIDOC_APPL INTO lr_data.
